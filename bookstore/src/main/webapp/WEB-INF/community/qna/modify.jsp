...
<div class="container-md mt-3 py-h">
    <h1 style="text-align: center">QnA 등록</h1>
    <form name="frmModify" id="frmModify" method="post" action="/community/qna/modify">
        <input type="hidden" name="idx" id="idx" value="${qnaDTO.idx}" />
        <div class="mb-3">
            <label for="title" class="form-label">제목</label>
            <input type="text" class="form-control" id="title" name="title" value="${qnaDTO.title}" placeholder="제목">
            <div id="div_err_title" style="display: none"></div>
        </div>

        <div class="mb-3">
            <label for="writer" class="form-label">작성자</label>
            <input type="text" class="form-control" id="writer" name="writer" value="${qnaDTO.writer}" readonly />
            <div id="div_err_writer" style="display: none;"></div>
        </div>

        <div class="mb-3">
            <label for="content" class="form-label">내용</label>
            <textarea class="form-control" id="content" name="content" rows="3" placeholder="내용을 입력하세요.">${qnaDTO.content}</textarea>
            <div id="div_err_content" style="display: none;"></div>
        </div>

        <div class="d-grid gap-2">
            <button class="btn btn-primary" type="submit">글 수정</button>
            <button class="btn btn-secondary" type="button" onclick="location.href='/community/view?type=qna&idx=${qnaDTO.idx}'">취소</button>
        </div>
    </form>
</div>
<script>
